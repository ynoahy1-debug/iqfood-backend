...
export default function AreasManager({ initialAreas }: { initialAreas: AreaItem[] }) {
  ...
  const handleDelete = async (area: AreaItem) => {
    if (!confirm(`هل أنت متأكد من حذف منطقة "${area.name}"؟`)) return;
  ...
  const totalRestaurantsInAreas = areas.reduce((acc, curr) => acc + (curr._count?.restaurants || 0), 0);
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          >
            إجمالي المناطق: <strong style={{ color: '#fff' }}>{areas.length}</strong>
          </span>
          <span
            style={{
              background: '#1e293b',
          ...
          >
            إجمالي المطاعم المصنفة بالمناطق: <strong style={{ color: '#fff' }}>{totalRestaurantsInAreas}</strong>
          </span>
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right' }}>
          <thead>
            <tr style={{ background: '#0f172a', borderBottom: '1px solid #334155' }}>
              ...
              <th style={{ padding: '16px 20px', color: '#94a3b8', fontSize: 13, fontWeight: 600 }}>المدينة / المحافظة</th>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, borderBottom: '1px solid #334155', paddingBottom: 12 }}>
              <h2 style={{ fontSize: 18, color: '#38bdf8', margin: 0 }}>
                {editingArea ? '✏️ تعديل منطقة في بغداد' : '📍 إضافة منطقة / حي جديد'}
              </h2>
            ...
            <form onSubmit={handleSave}>
              <div style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 'bold', marginBottom: 6, color: '#cbd5e1' }}>
                  اسم المنطقة / الحي (مثال: الكرادة، المنصور، شارع فلسطين): *
                </label>
                <input
                  type="text"
                  required
