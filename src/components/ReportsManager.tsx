      ...
      : [
          {
            ...
            reviewId: 'rev_123',
            reason: 'محتوى غير ملائم يحتوي على كلمات بذيئة وإساءة ⚠️',
            status: 'PENDING',
  ...
  const handleResolve = async (id: string, actionType: 'DELETE_POST' | 'FREEZE_USER' | 'DISMISS') => {
    ...
    try {
      if (actionType === 'DELETE_POST') {
        alert('تم حذف المنشور المحتوى المسيء نهائياً من التطبيق! 🗑️');
      } else if (actionType === 'FREEZE_USER') {
        alert('تم تجميد وإيقاف حساب الناشر المسيء فورياً! ❄️');
      }
